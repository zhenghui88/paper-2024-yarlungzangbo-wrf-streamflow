...
TIMEREF = datetime(2000, 1, 1)


def get_timestamp(ncfile: Path):
    timestamp: list[datetime] = []
    with nc.Dataset(ncfile) as ncf:
        for dtvec in ncf.variables["Times"][:, :]:
            dtstr = dtvec.tobytes().decode("ascii")
            timestamp.append(datetime.fromisoformat(dtstr))
    return timestamp


def get_filelist(srcdir: Path, prefix: str):
    files: dict[datetime, Path] = {}
    for f in srcdir.glob(f"{prefix}*"):
        for t in get_timestamp(f):
            files[t] = f
    files = dict(sorted(files.items(), key=lambda t: t[0]))
    return files


def get_latlon(ncfile: Path):
    with nc.Dataset(ncfile) as ncf:
...
def get_variable(cdt: datetime, cfile: Path):
    with nc.Dataset(cfile) as ncf:
        ncf.set_auto_mask(False)
        for ii, dtvec in enumerate(ncf.variables["Times"][:, :]):
            dtstr = dtvec.tobytes().decode("ascii")
            if cdt == datetime.fromisoformat(dtstr):
                return (
                    ...
                    ncf.variables["SNOW"][ii, ...],
                    ncf.variables["ECAN"][ii, ...],
                    ncf.variables["EDIR"][ii, ...],
                    ncf.variables["ETRAN"][ii, ...],
                    ncf.variables["SMOIS"][ii, :, :, :],
                )
        else:
            return (
                None,
...
def extract(desfile: Path, srcdir: Path, prefix: str = "wrfout_d01_"):
    ...
    snow = np.empty((nsn, nwe))
    ecan = np.empty((nsn, nwe))
    edir = np.empty((nsn, nwe))
    etran = np.empty((nsn, nwe))
    smois = np.empty((4, nsn, nwe))
    with nc.Dataset(desfile, "w") as ncf:
        ...
        ncf.createDimension("soil_layers_stag", 4)
        ...
        ncf.createVariable(
            "SNOW", "f4", ("time", "south_north", "west_east"), zlib=True, complevel=6
        )
        ncf.variables["SNOW"].units = "kg m-2"
        ncf.variables["SNOW"].description = "snow water equivalent"
        ncf.createVariable(
            "ECAN", "f4", ("time", "south_north", "west_east"), zlib=True, complevel=6
        )
        ncf.variables["ECAN"].units = "kg m-2 s-1"
        ncf.variables["ECAN"].description = "canopy water evaporation"
        ncf.createVariable(
            "EDIR", "f4", ("time", "south_north", "west_east"), zlib=True, complevel=6
        )
        ncf.variables["EDIR"].units = "kg m-2 s-1"
        ncf.variables["EDIR"].description = "direct soil evaporation"
        ncf.createVariable(
            "ETRAN", "f4", ("time", "south_north", "west_east"), zlib=True, complevel=6
        )
        ncf.variables["ETRAN"].units = "kg m-2 s-1"
        ncf.variables["ETRAN"].description = "transpiration"
        ncf.createVariable(
            "SMOIS",
            "f4",
        ...
        ncf.variables["SMOIS"].units = "m3 m-3"
        ncf.variables["SMOIS"].description = "soil moisture content"
        ...
        for idt, (cdt, cfile) in enumerate(srcfiles.items()):
            ...
            ncf.variables["SNOW"][idt, ...] = snow
            ncf.variables["ECAN"][idt, ...] = ecan
            ncf.variables["EDIR"][idt, ...] = edir
            ncf.variables["ETRAN"][idt, ...] = etran
            ncf.variables["SMOIS"][idt, :, :, :] = smois
...
if __name__ == "__main__":
    ...
    parser.add_argument("desfile", type=Path, help="the output netCDF4 file")
    parser.add_argument(
        "srcdir",
        type=Path,
