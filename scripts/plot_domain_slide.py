...
PGAUGE_LOCATION_FILE = DATAROOT.joinpath("gauge_p_loc.csv")
...
def read_precip_gauge(filepath: Path) -> list[tuple[float, float]]:
...
# %%
pgauge_location = read_precip_gauge(PGAUGE_LOCATION_FILE)
...
for gaugecode, gaugeinfo in gauge_topo.items():
    ...
    ax.plot(gaugeinfo["longitude"], gaugeinfo["latitude"], "ro", markersize=5)
    ax.text(
        gaugeinfo["longitude"],
        gaugeinfo["latitude"],
...
for pgaugelat, pgaugelon in pgauge_location:
    ax.plot(pgaugelon, pgaugelat, "w.", markersize=3)

cb = plt.colorbar(
    ax=ax, mappable=cf, orientation="vertical", aspect=30, pad=0.01, shrink=0.6
)
...
fig.savefig(
    FIGROOT.joinpath("domain_slide").with_suffix(FIGSUFFIX).as_posix(),
    bbox_inches="tight",
)
