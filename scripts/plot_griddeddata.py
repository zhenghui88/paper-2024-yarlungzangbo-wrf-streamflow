...
# %%
fig = plt.figure(figsize=(7 / 2.54, 11.4 / 2.54), dpi=FIGDPI, layout="constrained")

axs = fig.subplots(5, 1, sharex=True, sharey=True, squeeze=False)
...
ax.contourf(
    rnwrf_clim.lon,
    rnwrf_clim.lat,
    np.maximum(100 * rnwrf_clim.std("ensemble") / rnwrf_clim.mean("ensemble"), 0.0),
    levels=levels,
    extend="max",
)
plot_yj(ax)
...
cf = ax.contourf(
    ...
    np.maximum(
        100
        * rnwrf_clim.sel(ensemble=[8, 13]).std("ensemble")
        / rnwrf_clim.mean("ensemble"),
        0.0,
    ),
    levels=levels,
...
cf = ax.contourf(
    ...
    prwrf_clim.lat,
    100
    * prwrf_clim.sel(ensemble=[8, 13]).std("ensemble")
    / prwrf_clim.mean("ensemble"),
    levels=levels,
    extend="max",
)
...
levels = np.linspace(0, 1.0, 11) * 100.0
...
cf = ax.contourf(
    ...
    etwrf_clim.lat,
    100
    * etwrf_clim.sel(ensemble=[8, 13]).std("ensemble")
    / etwrf_clim.mean("ensemble"),
    levels=levels,
    extend="max",
)
...
ax.contourf(
    ...
    pretdwrf_clim.lat,
    np.maximum(
        100 * pretdwrf_clim.std("ensemble") / pretdwrf_clim.mean("ensemble"), 0.0
    ),
    levels=levels,
    extend="max",
)
...
cf = ax.contourf(
    ...
    pretdwrf_clim.lat,
    100
    * pretdwrf_clim.sel(ensemble=[8, 13]).std("ensemble")
    / pretdwrf_clim.mean("ensemble"),
    levels=levels,
    extend="both",
)
...
cf = ax.contourf(
    ...
    smwrf_clim.lat,
    100
    * smwrf_clim.sel(ensemble=[8, 13]).std("ensemble")
    / smwrf_clim.mean("ensemble"),
    levels=levels,
    extend="max",
)
